...
# ─────────────────────────────────────────────
# 2️⃣ 병합 처리 (세종↔충남만 통합, 울산↔경남 통합 제거)
# ─────────────────────────────────────────────
combine_regions <- function(df) {
  # 🔹 울산·경남은 건드리지 않고 그대로 둠
  # 🔹 세종·충남만 통합 처리
  ...
  df_chungnam <- df %>%
    ...
    mutate(
      ...
      labor_force = ifelse(n_exist == 2, labor_force_sum, NA_real_),
      unemployed  = ifelse(n_exist == 2, unemployed_sum, NA_real_)
    ) %>%
    select(region, year, qtr, week_basis, labor_force, unemployed, ur)
  
  # 2) 나머지 지역 그대로 유지 (울산·경남 포함)
  df_keep <- df %>%
    filter(!region %in% c("충청남도", "세종특별자치시"))
...
# ─────────────────────────────────────────────
# 🔧 울산 1996·1997년 실업률 보정: 경상남도 값으로 대체
# ─────────────────────────────────────────────

years_to_fill <- c(1996, 1997)

for (yy in years_to_fill) {
  # 해당 연도에 울산 row가 있는지 확인
  has_ulsan <- any(unemp_annual$region == "울산광역시" & unemp_annual$year == yy)
  
  # 1) 경상남도 값 가져오기 (반드시 있어야 함)
  gnd_row <- unemp_annual %>%
    filter(region == "경상남도", year == yy)
  
  if (nrow(gnd_row) == 1) {
    if (!has_ulsan) {
      # 2-A) 울산 row 자체가 없으면 → 새로 추가
      unemp_annual <- bind_rows(
        unemp_annual,
        tibble(
          region     = "울산광역시",
          year       = yy,
          unemp_year = gnd_row$unemp_year
        )
      )
    } else {
      # 2-B) 울산 row는 있는데 unemp_year가 NA면 → 경남 값으로 대체
      unemp_annual <- unemp_annual %>%
        mutate(
          unemp_year = ifelse(
            region == "울산광역시" & year == yy & is.na(unemp_year),
            gnd_row$unemp_year,
            unemp_year
          )
        )
