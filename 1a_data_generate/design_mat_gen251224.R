...
make_Z <- function(pe_nums, P, cluster){
  ...
  # ★ 가장 중요: is_current를 (P*TT) 길이로 확장해서 곱해야 함 (recycling 방지)
  is_current_long <- rep(as.numeric(is_current_by_pe[as.character(pe_nums)]), each=P)
  ...
  if(any(is.na(idx_cpi$csi4) | is.na(idx_cpi$csi5))){
    warning("⚠ CPI/CSI 데이터에 NA가 있습니다. (cluster, pe_num) 매칭을 확인하세요.")
  }
  ...
  if(any(is.na(idx_unemp$unemp4) | is.na(idx_unemp$unemp5))){
    warning("⚠ 실업률 unemp4/unemp5에 NA가 있습니다. (cluster, pe_num) 매칭을 확인하세요.")
  }
  ...
  colnames(Z) <- c("home_dem","homeground_dem","home_con","homeground_con","cpi4","cpi5","unemp4","unemp5")
  Z
}
