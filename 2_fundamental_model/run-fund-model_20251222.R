...
# 가중평균을 0.5로 맞춘 초기 alpha: y0 - (w'y0) + 0.5
# (주의) 여기서 y0는 확률(0~1) 스케일이며, baseline 모델이 Normal로 관측하는 구조와 일치
Alpha_init <- y0 - sum(Pop_weight[, 1] * y0) + 0.5
...
# (2) GLMM logit (counts)
data_list_23$X_pred <- X_scaled[(N+1):(N+P),,drop=FALSE]
data_list_23$Z_pred <- Z_scaled[(N+1):(N+P),,drop=FALSE]

fit2 <- run_and_save(
  stan_file = "2_fundamental_model/fund-model-simple_GLMM1.stan",
  data_list = data_list_23,
  model_tag = "m2_glmm_logit",
  out_dir   = out_dir
)
